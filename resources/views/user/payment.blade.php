...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
	<form action="{{ route('payment.post') }}" method="post" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{env('STRIPE_KEY') }}" id="payment-form">
      ...
      @foreach ($orders as $order)
         <input type="hidden" name="sub" id="sub" value="{{$order->amount}}"></input>
         <input type="hidden" name="id" id="id" value="{{$order->id}}"></input>
      @endforeach 
               ...
               <div class="panel panel-default credit-card-box">
                  ...
                  <div class="panel-body">
                     <br>
                        ...
                        <div class="form-row row">
                           <div class="col-xs-12">
                              <button class="btn btn-primary btn-md btn-block mt-2" type="submit">Pay Now</button>
                           </div>
                        </div>

                     <div class='form-row row'>
                         <div class='col-md-12 error form-group visually-hidden-focusable'>
...
$(function() {
  ...
  $('form.require-validation').bind('submit', function(e) {
    ...
    valid = true;
    $errorMessage.addClass('visually-hidden-focusable');
    $('.has-error').removeClass('has-error');
    $inputs.each(function(i, el) {
        var $input = $(el);
        if ($input.val() === '') {
            $input.parent().addClass('has-error');
            $errorMessage.removeClass('visually-hidden-focusable');
            e.preventDefault();
        }
    });
